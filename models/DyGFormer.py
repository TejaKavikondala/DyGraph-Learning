...
class DyGFormer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 dropout: float = 0.1, max_input_sequence_length: int = 512, device: str = 'cpu', lambda_param : float = 0.1, centrality_scores = None):
        """
        DyGFormer model.
        ...
        if centrality_scores is not None:
            self.hierarchy_levels = torch.tensor([centrality_scores.get(node_id, 0) for node_id in range(len(node_raw_features))], dtype=torch.float32).to(device)
        else:
            self.hierarchy_levels = None
    ...
    def set_hierarchy_levels(self, graph):
        """
        Compute centrality scores for each node in the graph and set hierarchy levels.
        ...
        # Convert to tensor
        centrality_tensor = torch.tensor([centrality_scores[node] for node in sorted(graph.nodes())], dtype=torch.float32).to(self.device)
        self.hierarchy_levels = centrality_tensor

    def compute_centrality(self, graph):
        """
        Compute centrality scores for each node in the graph.
        :param graph: NetworkX graph
        :return: dictionary with node IDs as keys and centrality scores as values
        """
        if self.centrality_type == "degree":
            centrality_scores = nx.degree_centrality(graph)
        elif self.centrality_type == "betweenness":
        ...
        # Convert centrality scores to a tensor for use in PyTorch
        centrality_tensor = torch.tensor([centrality_scores[node] for node in sorted(graph.nodes())], dtype=torch.float32)
        return centrality_tensor.to(self.device)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, attention_dim: int, num_heads: int, dropout: float = 0.1, hierarchy_levels = None):
        """
        Transformer encoder.
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.hierarchy_levels is not None:
            # Replicate hierarchy_levels across the sequence length (num_patches) dimension
            batch_size, num_patches, attention_dim = inputs.size()
            
            # Expand hierarchy_levels to match the batch and patch dimensions
            # Assume that hierarchy_levels has one entry per node and needs to be repeated for each patch
            bias = self.hierarchy_levels[:num_patches].unsqueeze(0).unsqueeze(-1).expand(batch_size, num_patches, attention_dim)
